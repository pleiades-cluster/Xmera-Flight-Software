...
class Field():
    ...
    def __init__(self,cdh,debug):
        self.debug=debug
        self.impacketcount=0
        self.cdh=cdh
        self.cdh.radio1.spreading_factor=8
        self.cdh.radio1.tx_power=23
        self.cdh.radio1.low_datarate_optimize=False
        self.cdh.radio1.node=0xfb
        self.cdh.radio1.destination=0xfa
        self.cdh.radio1.receive_timeout=10
        self.cdh.radio1.enable_crc=True
        if self.cdh.radio1.spreading_factor>8:
            self.cdh.radio1.low_datarate_optimize=True
    ...
    def fieldSide(self,msg,packlow,packhigh):
        ...
        self.cdh.radio1.send("packet" + pLow + "/" + pHigh + ": " + msg, keep_listening=True)

        if self.debug: print("Listening for transmissions, " + str(self.cdh.radio1.receive_timeout))
        heard_something = self.cdh.radio1.await_rx(timeout=20)

        if heard_something:
            response = self.cdh.radio1.receive(keep_listening=True)

            if response is not None :
                response_string = ''.join([chr(b) for b in response])
                if response_string == "True":
                    if self.debug: print("packet received")
                    if self.debug: print('msg: {}, RSSI: {}'.format(response_string,self.cdh.radio1.last_rssi-137))
                    return True
