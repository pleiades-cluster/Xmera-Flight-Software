...
class Face:
    ...
    def __init__(self, Add, Pos, debug_state):
        ...
        if Pos == "x+":
            ...
            if self.debug: print("[ERROR] Please input a propper face")
    ...
    def Sensorinit(self,senlist,address):
        ...
        if "MCP" in senlist:
            try:
                ...
                if self.debug: print('[ACTIVE][Temperature Sensor]')
            except Exception as e:
                if self.debug: print('[ERROR][Temperature Sensor]',e)
        ...
        if "ADPS" in senlist:
            try:
                ...
                if self.debug: print('[ACTIVE][Light Sensor]')
            except Exception as e: 
                if self.debug: print('[ERROR][Light Sensor]',e)
        ...
        if "MAG" in senlist:            
            try:
                ...
                if self.debug: print('[ACTIVE][Magnetometer]')
            except Exception as e:
                if self.debug: print('[ERROR][Magnetometer]',e)
        ...
        if "IMU" in senlist:
            try:
                ...
                if self.debug: print('[ACTIVE][IMU]')
            except Exception as e: 
        ...
        if "DRV" in senlist:
            try:
                ...
                if self.debug: print('[ACTIVE][Motor Driver]')
            except Exception as e:
                if self.debug: print('[ERROR][Motor Driver]',e)
        ...
        if "COUPLE" in senlist:
            try:
                ...
                if self.debug: print('[ACTIVE][Thermocouple]')
            except Exception as e:
                if self.debug: print('[ERROR][Thermocouple]',e)

        if self.debug: print('Initialization Complete')
    ...
    def temperature(self): 
        if self.sensors['MCP']:
            ...
            if self.debug: print('[WARNING]Temperature sensor not initialized')
    ...
    def color_data(self): 
        if self.sensors['ADPS']:
            ...
            if self.debug: print('[WARNING]Light sensor not initialized')
    ...
    def mag(self): 
        if self.sensors['MAG']:
            ...
            if self.debug: print('[WARNING]Magnetometer not initialized')  
    ...
    def accel(self):
        if self.sensors['IMU']:
            ...
            if self.debug: print('[WARNING]IMU not initialized')   
    ...
    def drv_actuate(self, duration): 
        if self.sensors['DRV']:
            if self.debug: print('Actuating Sequence')
            if self.debug: print("Playing effect #{0}".format(self.drv))
            if self.debug & self.sensors['MAG']: print("X:%.2f, Y: %.2f, Z: %.2f uT"%(self.mag))
            self.drv1.play()
            if self.debug & self.sensors['MAG']: print("X:%.2f, Y: %.2f, Z: %.2f uT"%(self.mag))
            time.sleep(duration)
            if self.debug & self.sensors['MAG']: print("X:%.2f, Y: %.2f, Z: %.2f uT"%(self.mag))
            self.drv1.stop()
            if self.debug & self.sensors['MAG']: print("X:%.2f, Y: %.2f, Z: %.2f uT"%(self.mag))
            if self.debug: print('Actuation Complete')
        else: 
            if self.debug: print('[WARNING]Motor driver not initialized')
    ...
    def drv(self, sequence): 
        if self.sensors['DRV']:
            try:
                if self.debug: print('Encoding Sequence')
                self.drv1.sequence[0] = adafruit_drv2605.Effect(sequence)
                if self.debug: print('Complete')
            except Exception as e:
                if self.debug: print('[ERROR][Motor Driver]',e)
        else: 
            if self.debug: print('[WARNING]Motor driver not initialized')
    ...
    def couple(self): 
        if self.sensors['COUPLE']:
            ...
            if self.debug: print('[WARNING]Thermocouple not initialized')  
    ...
    def test_all(self, num, rate): 
        self.datalist=[]
        if self.debug: print('Expected Sensors: ', self.senlist_what)
        if self.debug: print('Initialized Sensors: ', self.active_sensors)
        time.sleep(1) #Remove later for performance boost! 
        if self.debug: print('Initializing Test')

        for i in range(num): 

            if self.debug: print('Test Number: ', i+1, ' /', num)

            #Test Temperature Sensor
            if self.debug: print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
            if ("MCP" in self.senlist) and (self.sensors.get("MCP") == True):
                try:
                    if self.debug: print('Temperature Sensor')
                    if self.debug: print('Face Temperature: ', self.temperature)
                    self.datalist.append(self.temperature)
                except Exception as e:
                    if self.debug: print('[ERROR][Temperature Sensor]',e)
            else:
                if self.debug: print('[ERROR]Temperature Sensor Failure')
                
            if self.debug: print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
            #Test Light Sensor
            if ("ADPS" in self.senlist) and (self.sensors.get('ADPS') == True ):
                try:
                    if self.debug: print('Light Sensor')
                    if self.debug: print(self.color_data)
                    self.datalist.append(self.color_data)
                except Exception as e: 
                    if self.debug: print('[ERROR][Light Sensor]',e)
            else:
                if self.debug: print('[ERROR]Light Sensor Failure')
                
            if self.debug: print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
            #Test Magnetometer
            if ("MAG" in self.senlist) and (self.sensors.get('MAG') == True ):            
                try:
                    if self.debug: print('Magnetometer')
                    if self.debug: print("X:%.2f, Y: %.2f, Z: %.2f uT"%(self.mag))
                ...
                if self.debug: print('[Error]Magnetometer Failure')
                
            #Test IMU
            if ("IMU" in self.senlist) and (self.sensors.get('IMU') == True ):            
                try:
                    if self.debug: print('IMU')
                    if self.debug: print("Acceleration: X:%.2f, Y: %.2f, Z: %.2f m/s^2"%(self.imu.acceleration))
                    if self.debug: print("Gyro X:%.2f, Y: %.2f, Z: %.2f radians/s"%(self.imu.gyro))
                ...
                if self.debug: print('[Error]IMU Failure')
      
            #Test Motor Driver
            if ("DRV" in self.senlist) and (self.sensors.get('DRV') == True ):
                try:
                    if self.debug: print('Motor Driver')
                    if self.debug: print('[ACTIVE][Motor Driver]') #No function defined here yet to use the driver
                except Exception as e:
                    if self.debug: print('[ERROR][Motor Driver]',e)
                if self.debug: print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~') 
            else:
            ...
            if ("COUPLE" in self.senlist) and (self.sensors.get('COUPLE') == True ):
                try:
                    if self.debug: print('Thermocouple')
                    if self.debug: print(self.couple) #Unformatted
                    self.datalist.append(self.couple)
                except Exception as e:
                    if self.debug: print('[ERROR][Thermocouple]',e)
                if self.debug: print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~') 
            elif self.position == "z-":
                if self.debug: print('[ERROR]Thermocouple Failure')
            else:
                pass
